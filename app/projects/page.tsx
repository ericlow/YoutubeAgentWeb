    ...
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      ...
      <main className="container mx-auto px-4 py-4 md:py-8">
        ...
        <div className="flex items-center justify-between mb-4 md:mb-8">
          ...
          <Button className="gap-2 bg-[#007AFF] hover:bg-[#0051D5] text-white border-0 ml-auto md:ml-0">
        ...
        <div className="relative mb-4 md:mb-8">
          ...
          <Input
            ...
            className="pl-10 h-11 md:h-12 focus-visible:ring-2 focus-visible:ring-blue-500 text-base"
          />
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {filteredProjects.map((project) => (
              <Link key={project.id} href={`/projects/${project.id}`}>
                <Card className="p-5 md:p-6 bg-blue-50/50 hover:bg-blue-50 hover:shadow-lg transition-all cursor-pointer group border-blue-100 active:scale-[0.98]">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div
                        className={`w-10 h-10 md:w-10 md:h-10 rounded-lg ${project.color} flex items-center justify-center flex-shrink-0`}
                      >
                        <FolderOpen className="w-5 h-5 text-white" />
                      </div>
                      <h3 className="text-base md:text-lg font-semibold text-balance truncate">{project.name}</h3>
                    </div>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild onClick={(e) => e.preventDefault()}>
                        <Button
                          variant="ghost"
                          size="icon"
                          className="opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity flex-shrink-0 h-9 w-9"
                        >
